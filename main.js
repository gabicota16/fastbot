...
global.authFile = `FastBotSession`;
...
if (opts['autocleartmp'] && (global.support || {}).find) (tmp = [os.tmpdir(), 'tmp', 'FastJadiBot'], tmp.forEach((filename) => cp.spawn('find', [filename, '-amin', '3', '-type', 'f', '-delete'])));
}, 30 * 1000);
}}
...
let prekey = []
let directorio = readdirSync("./FastBotSession")
let filesFolderPreKeys = directorio.filter(file => {
return file.startsWith('pre-key-') || file.startsWith('session-') || file.startsWith('sender-') || file.startsWith('app-') 
})
prekey = [...prekey, ...filesFolderPreKeys]
filesFolderPreKeys.forEach(files => {
unlinkSync(`./FastBotSession/${files}`)
})
} 

function purgeSessionSB() {
try {
let listaDirectorios = readdirSync('./FastJadiBot/');
let SBprekey = []
listaDirectorios.forEach(directorio => {
if (statSync(`./FastJadiBot/${directorio}`).isDirectory()) {
let DSBPreKeys = readdirSync(`./FastJadiBot/${directorio}`).filter(fileInDir => {
return fileInDir.startsWith('pre-key-') /*|| fileInDir.startsWith('app-') || fileInDir.startsWith('session-')*/
})
SBprekey = [...SBprekey, ...DSBPreKeys]
DSBPreKeys.forEach(fileInDir => {
unlinkSync(`./FastJadiBot/${directorio}/${fileInDir}`)
})
}
})
...
const directories = ['./FastBotSession/', './FastJadiBot/']
